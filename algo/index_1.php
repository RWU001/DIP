<?php
	...
	*/
	...
	for ($work_i = 0; $work-i < $num_worker; $work_i++) {
		array_push($questionstring,[[],[]]);
		$questionstring[i][0] = "A.";
		$questionstring[i][1] = "B.";
		foreach($question[i][0] as $w)
		$questionstring[i][0] .= " " . "$multi_class[w]";
		foreach($question[i][1] as $w)
		$questionstring[i][1] .= " " . "$multi_class[w]";
	}
